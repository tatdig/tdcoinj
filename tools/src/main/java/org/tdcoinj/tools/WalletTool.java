...
import org.tdcoinj.wallet.DeterministicSeed;
...
import org.tdcoinj.core.FullPrunedBlockChain;
...
import org.tdcoinj.core.listeners.BlocksDownloadedEventListener;
import org.tdcoinj.core.listeners.DownloadProgressTracker;
import org.tdcoinj.wallet.MarriedKeyChain;
import org.tdcoinj.wallet.Protos;
import org.tdcoinj.wallet.SendRequest;
import org.tdcoinj.wallet.Wallet;
import org.tdcoinj.wallet.WalletExtension;
...
import static org.tdcoinj.core.Coin.parseCoin;
import static com.google.common.base.Preconditions.checkNotNull;

/**
 * A command line tool for manipulating wallets and working with Tdcoin.
 */
public class WalletTool {
    ...
    private static org.tdcoin.protocols.payments.Protos.PaymentRequest paymentRequest;
    ...
    public static class Condition {
        ...
        public boolean matchTdcoins(Coin comparison) {
            try {
    ...
    private static void sendPaymentRequest(String location, boolean verifyPki) {
        if (location.startsWith("http") || location.startsWith("tdcoin")) {
            try {
                ListenableFuture<PaymentSession> future;
                if (location.startsWith("http")) {
                    future = PaymentSession.createFromUrl(location, verifyPki);
                } else {
                    TdcoinURI paymentRequestURI = new TdcoinURI(location);
                    future = PaymentSession.createFromTdcoinUri(paymentRequestURI, verifyPki);
                }
                ...
                System.err.println("Invalid tdcoin uri: " + e.getMessage());
                System.exit(1);
            } catch (InterruptedException e) {
                // Ignore.
            } catch (ExecutionException e) {
            ...
            try {
                paymentRequest = org.tdcoin.protocols.payments.Protos.PaymentRequest.newBuilder().mergeFrom(stream).build();
            } catch(IOException e) {
    ...
    private static void wait(WaitForEnum waitFor) throws BlockStoreException {
        ...
        switch (waitFor) {
            ...
            case BALANCE:
                ...
                if (condition.matchTdcoins(wallet.getBalance(Wallet.BalanceType.ESTIMATED))) {
                    latch.countDown();
                    break;
                }
    ...
    private static void deleteKey() {
        ...
        if (pubKey != null) {
            ...
            try {
                ...
                System.err.println(addr + " does not parse as a Tdcoin address of the right network parameters.");
                return;
            }
        }
        if (key == null) {
    ...
    static synchronized void onChange(final CountDownLatch latch) {
        ...
        if (condition.matchTdcoins(balance)) {
