/*
 * Copyright 2014 The tdcoinj team
 *
...
/** Takes a URL or tdcoin URI and prints information about the payment request. */
public class PaymentProtocolTool {
    public static void main(String[] args) {
        if (args.length < 1) {
            System.err.println("Provide a tdcoin URI or URL as the argument.");
            return;
        }
        dump(args[0]);
    }
    ...
    private static void dump(String arg) {
        try {
            ...
            } else if ("tdcoin".equals(uri.getScheme())) {
                TdcoinURI bcuri = new TdcoinURI(arg);
                final String paymentRequestUrl = bcuri.getPaymentRequestUrl();
                if (paymentRequestUrl == null) {
                    System.err.println("No r= param in tdcoin URI");
                    return;
                }
                session = PaymentSession.createFromTdcoinUri(bcuri).get();
            } else {
            ...
            StringBuilder output = new StringBuilder(
                    format("Tdcoin payment request, version %d%nDate: %s%n", version, session.getDate()));
