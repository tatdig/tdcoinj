...
import org.tdcoinj.core.BloomFilter;
...
import org.tdcoinj.core.PeerFilterProvider;
...
import org.tdcoinj.core.TransactionBag;
import org.tdcoinj.core.TransactionBroadcast;
import org.tdcoinj.core.TransactionBroadcaster;
...
import org.tdcoinj.core.UTXOProvider;
import org.tdcoinj.core.UTXOProviderException;
import org.tdcoinj.core.Utils;
import org.tdcoinj.core.VerificationException;
import org.tdcoinj.core.TransactionConfidence.*;
import org.tdcoinj.crypto.*;
import org.tdcoinj.script.*;
import org.tdcoinj.script.Script.ScriptType;
import org.tdcoinj.signers.*;
import org.tdcoinj.utils.*;
import org.tdcoinj.wallet.Protos.Wallet.*;
import org.tdcoinj.wallet.WalletTransaction.*;
import org.tdcoinj.wallet.listeners.CurrentKeyChangeEventListener;
 ...
 * of the Tdcoin protocol.</p>
 *
 * <p>To learn more about this class, read <b><a href="https://tdcoinj.github.io/working-with-the-wallet">
 ...
 * other objects, see the <a href="https://tdcoinj.github.io/getting-started">Getting started</a> tutorial
...
public class Wallet extends BaseTaggableObject
    ...
    // Dead:     Transactions that we believe will never confirm get moved here, out of pending. Note that Tdcoin
     ...
     * key, after loading a wallet produced by previous versions of tdcoinj. If the wallet is encrypted the key
     ...
     * @throws org.tdcoinj.core.ECKey.MissingPrivateKeyException if the seed is unavailable (watching wallet)
     */
    ...
    private void processTxFromBestChain(Transaction tx, boolean forceAddToPool) throws VerificationException {
        ...
        if (isDeadCoinbase) {
            ...
            // we could. Tdcoin Core nodes on the network have deleted the dependent transactions from their mempools
        ...
        for (TransactionInput input : tx.getInputs()) {
            ...
            if (result == TransactionInput.ConnectionResult.ALREADY_SPENT) {
                if (fromChain) {
                    ...
                    log.warn("{}: {}", tx.getTxId(), Utils.HEX.encode(tx.unsafeTdcoinSerialize()));
                    Transaction other = output.getSpentBy().getParentTransaction();
                    log.warn("{}: {}", other.getTxId(), Utils.HEX.encode(other.unsafeTdcoinSerialize()));
                }
    ...
    /**
     * Returns the amount of tdcoin ever received via output. <b>This is not the balance!</b> If an output spends from a
    ...
    /**
     * Returns the amount of tdcoin ever sent via output. If an output is sent to our own wallet, because of change or
    ...
    public static class SendResult {
        /** The Tdcoin transaction message that moves the money. */
     ...
     *
     * @param address The Tdcoin address to send the money to.
     ...
     * @throws ExceededMaxTransactionSize if the resultant transaction is too big for Tdcoin to process.
    ...
    /**
     * Thrown if the resultant transaction is too big for Tdcoin to process. Try breaking up the amounts of value.
     */
    ...
    public void completeTx(SendRequest req) throws InsufficientMoneyException, BadWalletEncryptionKeyException {
        ...
        try {
            ...
            // Check size.
            final int size = req.tx.unsafeTdcoinSerialize().length;
            ...
            for (Sha256Hash blockHash : oldBlockHashes) {
                for (TxOffsetPair pair : mapBlockTx.get(blockHash)) {
                    ...
                    if (tx.isCoinBase()) {
                        ...
                        // this coinbase tx. Some can just go pending forever, like the Tdcoin Core. However we
                        // can do our best.
        ...
        while (true) {
            ...
            for (int i = 0; i < req.tx.getOutputs().size(); i++) {
                TransactionOutput output = new TransactionOutput(params, tx,
                        req.tx.getOutputs().get(i).tdcoinSerialize(), 0);
                if (req.recipientsPayFees) {
    ...
    private void addSuppliedInputs(Transaction tx, List<TransactionInput> originalInputs) {
        for (TransactionInput input : originalInputs)
            tx.addInput(new TransactionInput(params, tx, input.tdcoinSerialize()));
    }
     ...
     * sending transactions to the Tdcoin network by default. Some sendCoins methods let you specify a broadcaster
     ...
     */
    public void setTransactionBroadcaster(@Nullable org.tdcoinj.core.TransactionBroadcaster broadcaster) {
     ...
     * @throws org.tdcoinj.wallet.DeterministicUpgradeRequiresPassword if key rotation requires the users password.
     */
    ...
    private Transaction rekeyOneBatch(long timeSecs, @Nullable KeyParameter aesKey, List<Transaction> others, boolean sign) {
        ...
        try {
            ...
            checkState(rekeyTx.unsafeTdcoinSerialize().length < Transaction.MAX_STANDARD_TX_SIZE);
            return rekeyTx;
