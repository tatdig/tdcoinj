...
import org.tdcoinj.net.AbstractTimeoutHandler;
import org.tdcoinj.net.NioClient;
import org.tdcoinj.net.NioClientManager;
import org.tdcoinj.net.StreamConnection;
import org.tdcoinj.store.BlockStore;
import org.tdcoinj.store.BlockStoreException;
import org.tdcoinj.utils.ListenerRegistration;
...
/**
 * <p>A Peer handles the high level communication with a Tdcoin node, extending a {@link PeerSocketHandler} which
...
public class Peer extends PeerSocketHandler {
    ...
    private void processVersionMessage(VersionMessage m) throws ProtocolException {
        ...
        log.info(toString());
        // tdcoinj is a client mode implementation. That means there's not much point in us talking to other client
        ...
        if ((vPeerVersionMessage.localServices
                & VersionMessage.NODE_TDCOIN_CASH) == VersionMessage.NODE_TDCOIN_CASH) {
    ...
    protected void processAlert(AlertMessage m) {
        try {
            ...
            // BigInteger implementations! See https://github.com/tdcoinj/tdcoinj/issues/526 for discussion. As
        ...
        try {
            ...
            try {
                if (awaitingFreshFilter != null) {
                    ...
                    // range of blocks and get an inv both times, due to the codepath in Tdcoin Core hitting
            ...
            if (blocks.size() > 0 && downloadData && blockChain != null) {
                ...
                for (InventoryItem item : blocks) {
                    if (blockChain.isOrphan(item.hash) && downloadBlockBodies) {
                        ...
                        //
                        // Note that as of June 2012 Tdcoin Core won't actually ever interleave blocks pushed as
                        // part of chain download with newly announced blocks, so it should always be taken care of by
                        // the duplicate check in blockChainDownloadLocked(). But Tdcoin Core may change in future so
    ...
    private void blockChainDownloadLocked(Sha256Hash toHash) {
        ...
        //
        //   https://en.tdcoin.it/wiki/Protocol_specification#getblocks
        //
     ...
     * after the query returns. <b>Most peers do not support this request. You will need to connect to Tdcoin XT
     ...
     * <b>Most peers do not support this request. You will need to connect to Tdcoin XT peers if you want
     * this to work.</b>
     *
    ...
    public ListenableFuture<UTXOsMessage> getUTXOs(List<TransactionOutPoint> outPoints, boolean includeMempool) {
        ...
        try {
            ...
            if ((peerVer.localServices & GetUTXOsMessage.SERVICE_FLAGS_REQUIRED) != GetUTXOsMessage.SERVICE_FLAGS_REQUIRED)
                throw new ProtocolException("Peer does not support getutxos protocol flag: find Tdcoin XT nodes.");
