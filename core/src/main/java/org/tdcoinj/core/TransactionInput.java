...
import org.tdcoinj.wallet.DefaultRiskAnalysis;
import org.tdcoinj.wallet.KeyBag;
import org.tdcoinj.wallet.RedeemData;
...
public class TransactionInput extends ChildMessage {
    ...
    protected void tdcoinSerializeToStream(OutputStream stream) throws IOException {
        outpoint.tdcoinSerialize(stream);
     ...
     * in nodes memory pools if the existing version is time locked. See the Contracts page on the Tdcoin wiki for
     ...
     * <a href="https://github.com/tdcoin/bips/blob/master/bip-0068.mediawiki">relative lock-time</a> enabled.
     */
    ...
    public TransactionInput duplicateDetached() {
        return new TransactionInput(params, null, tdcoinSerialize(), 0);
    }

    /**
     * <p>Returns either RuleViolation.NONE if the input is standard, or which rule makes it non-standard if so.
     * The "IsStandard" rules control whether the default Tdcoin Core client blocks relay of a tx / refuses to mine it,
