...
/**
 * <p>A Message is a data structure that can be serialized/deserialized using the Tdcoin serialization format.
...
public abstract class Message {
     ...
     * @param protocolVersion Tdcoin protocol version.
    ...
    private void selfCheck(byte[] payload, int offset) {
        if (!(this instanceof VersionMessage)) {
            ...
            byte[] reserialized = tdcoinSerialize();
    ...
    // These methods handle the serialization/deserialization using the custom Tdcoin protocol.
    ...
    /**
     * Returns a copy of the array returned by {@link Message#unsafeTdcoinSerialize()}, which is safe to mutate.
    ...
    public byte[] tdcoinSerialize() {
        byte[] bytes = unsafeTdcoinSerialize();
    ...
    /**
     * <p>Serialize this message to a byte array that conforms to the tdcoin wire protocol.</p>
     *
     ...
     */
    public byte[] unsafeTdcoinSerialize() {
        ...
        try {
            tdcoinSerializeToStream(stream);
        } catch (IOException e) {
        ...
        if (serializer.isParseRetainMode()) {
            ...
            // which means another call to tdcoinSerialize is coming.  If we didn't recache then internal
    ...
    /**
     * Serialize this message to the provided OutputStream using the tdcoin wire format.
     *
     * @param stream
     * @throws IOException
     */
    public final void tdcoinSerialize(OutputStream stream) throws IOException {
    ...
    /**
     * Serializes this message to the provided stream. If you just want the raw bytes use tdcoinSerialize().
     */
    protected void tdcoinSerializeToStream(OutputStream stream) throws IOException {
        log.error("Error: {} class has not implemented tdcoinSerializeToStream method.  Generating message with no payload", getClass());
    }

    /**
     * This method is a NOP for all classes except Block and Transaction.  It is only declared in Message
     * so TdcoinSerializer can avoid 2 instanceof checks + a casting.
     */
