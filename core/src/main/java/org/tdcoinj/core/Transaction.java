...
import static org.tdcoinj.core.Utils.*;
 ...
 * the minting of new coins. A Transaction object corresponds to the equivalent in the Tdcoin C++ implementation.</p>
 *
 * <p>Transactions are the fundamental atoms of Tdcoin and have many powerful features. Read
 * <a href="https://tdcoinj.github.io/working-with-transactions">"Working with transactions"</a> in the
 * documentation to learn more about how to use this class.</p>
 *
 * <p>All Tdcoin transactions are at risk of being reversed, though the risk is much less than with traditional payment
...
public class Transaction extends ChildMessage {
    ...
    /**
     * If feePerKb is lower than this, Tdcoin Core will treat it as if there were no fee.
     */
    public static final Coin REFERENCE_DEFAULT_MIN_TX_FEE = Coin.valueOf(1000); // 0.01 mTDC

    /**
     * If using this feePerKb, transactions will get confirmed within the next couple of blocks.
     * This should be adjusted from time to time. Last adjustment: February 2017.
     */
    public static final Coin DEFAULT_TX_FEE = Coin.valueOf(100000); // 1 mTDC
     ...
     * @param payload Tdcoin protocol formatted byte array containing message content.
     ...
     * @param hashFromHeader Used by TdcoinSerializer. The serializer has to calculate a hash for checksumming so to
    ...
    public Sha256Hash getTxId() {
        if (cachedTxId == null) {
            if (!hasWitnesses() && cachedWTxId != null) {
                ...
                try {
                    tdcoinSerializeToStream(stream, false);
                } catch (IOException e) {
    ...
    public Sha256Hash getWTxId() {
        if (cachedWTxId == null) {
            if (!hasWitnesses() && cachedTxId != null) {
                ...
                try {
                    tdcoinSerializeToStream(baos, hasWitnesses());
                } catch (IOException e) {
                    throw new RuntimeException(e); // cannot happen
                }
    ...
    public int getWeight() {
        ...
        try (final ByteArrayOutputStream stream = new UnsafeByteArrayOutputStream(length)) {
            ...
            stream.reset();
            tdcoinSerializeToStream(stream, true);
    ...
    /**
     * Deserialize according to <a href="https://github.com/tdcoin/bips/blob/master/bip-0144.mediawiki">BIP144</a> or
     * the <a href="https://en.tdcoin.it/wiki/Protocol_documentation#tx">classic format</a>, depending on if the
     ...
     * value is determined by a formula that all implementations of Tdcoin share. In 2011 the value of a coinbase
    ...
    public String toString(@Nullable AbstractBlockChain chain, @Nullable CharSequence indent) {
        ...
        int size = unsafeTdcoinSerialize().length;
        int vsize = getVsize();
    ...
    public void clearInputs() {
        ...
        this.length = this.unsafeTdcoinSerialize().length;
    }
     ...
     * and requires external synchronization. Please refer to general documentation on Tdcoin scripting and contracts
    ...
    public Sha256Hash hashForSignature(int inputIndex, byte[] connectedScript, byte sigHashType) {
        ...
        //
        //   https://en.tdcoin.it/wiki/Contracts

        try {
            // Create a copy of this transaction to operate upon because we need make changes to the inputs and outputs.
            // It would not be thread-safe to change the attributes of the transaction object itself.
            Transaction tx = this.params.getDefaultSerializer().makeTransaction(this.tdcoinSerialize());
            ...
            // This step has no purpose beyond being synchronized with Tdcoin Core's bugs. OP_CODESEPARATOR
            // is a legacy holdover from a previous, broken design of executing scripts that shipped in Tdcoin 0.1.
            ...
            // Set the input to the script of its output. Tdcoin Core does this but the step has no obvious purpose as
            ...
            if ((sigHashType & 0x1f) == SigHash.NONE.value) {
                ...
                if (inputIndex >= tx.outputs.size()) {
                    ...
                    // Tdcoin implementation. Tdcoin Core also contains a bug in handling this case:
                    // any transaction output that is signed in this case will result in both the signed output
                    // and any future outputs to this public key being steal-able by anyone who has
                    // the resulting signature and the public key (both of which are part of the signed tx input).

                    // Tdcoin Core's bug is that SignatureHash was supposed to return a hash and on this codepath it
            ...
            tx.tdcoinSerializeToStream(bos, false);
     ...
     * scriptPubKey of the output you're signing for. (See BIP143: https://github.com/tdcoin/bips/blob/master/bip-0143.mediawiki)</p>
     *
     * @param inputIndex   input the signature is being calculated for. Tx signatures are always relative to an input.
    ...
    @Override
    protected void tdcoinSerializeToStream(OutputStream stream) throws IOException {
        boolean useSegwit = hasWitnesses()
                && protocolVersion >= NetworkParameters.ProtocolVersion.WITNESS_VERSION.getTdcoinProtocolVersion();
        tdcoinSerializeToStream(stream, useSegwit);
    }

    /**
     * Serialize according to <a href="https://github.com/tdcoin/bips/blob/master/bip-0144.mediawiki">BIP144</a> or the
     * <a href="https://en.tdcoin.it/wiki/Protocol_documentation#tx">classic format</a>, depending on if segwit is
     * desired.
     */
    protected void tdcoinSerializeToStream(OutputStream stream, boolean useSegwit) throws IOException {
        // version
        ...
        for (TransactionInput in : inputs)
            in.tdcoinSerialize(stream);
        // txout_count, txouts
        stream.write(new VarInt(outputs.size()).encode());
        for (TransactionOutput out : outputs)
            out.tdcoinSerialize(stream);
        // script_witnisses
        if (useSegwit) {
            for (TransactionInput in : inputs) {
                in.getWitness().tdcoinSerializeToStream(stream);
            }
        }
        // lock_time
     ...
     * since Tdcoin 0.8+ a transaction that did not end its lock period (non final) is considered to be non
     ...
     * (<a href="https://github.com/tdcoin/bips/blob/master/bip-0068.mediawiki">BIP 68</a>) if it is version 2 or
     ...
     * <a href="https://github.com/tdcoin/bips/blob/master/bip-0125.mediawiki">full replace-by-fee </a> semantics.
     */
     ...
     * This is useful in certain types of <a href="http://en.tdcoin.it/wiki/Contracts">contracts</a>, such as
     * micropayment channels.</p>
     *
     * <p>Note that currently the replacement feature is disabled in Tdcoin Core and will need to be
