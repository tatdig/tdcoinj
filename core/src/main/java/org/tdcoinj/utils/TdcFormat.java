...
import org.tdcoinj.utils.TdcAutoFormat.Style;
import static org.tdcoinj.utils.TdcAutoFormat.Style.*;
 ...
 * representations of Tdcoin monetary values.</p>
 *
 ...
 * represents a Tdcoin monetary value.  Some of the features offered for doing this
 * are:</p>
 *
 * <ol>
 ...
 *       {@link TdcFormat#format(Object)} method. 
 *   <li>Parse a value by passing a {@code String}-type
 *       representation of it to the {@link TdcFormat#parse(String)} method.</ol>
 *
 * <p>For example, depending on your locale, values might be formatted
 * and parsed as follows:</p>
 *
 * <blockquote><pre>
 * TdcFormat f = TdcFormat.getInstance();
 * String c = f.format(Coin.COIN);                <strong>// "TDC 1.00"</strong>
 * String k = f.format(Coin.COIN.multiply(1000)); <strong>// "TDC 1,000.00"</strong>
 * String m = f.format(Coin.COIN.divide(1000));   <strong>// "mTDC 1.00"</strong>
 ...
 * be formatted, {@link TdcAutoFormat}, and another that formats any value in units of a
 * fixed, specified denomination, {@link TdcFixedFormat}.</p>
 *
 ...
 * values of {@link TdcAutoFormat.Style}.  There are two styles constants: {@link
 * TdcAutoFormat.Style#CODE} (the default), and {@link TdcAutoFormat.Style#SYMBOL}.  The
 * difference is that the {@code CODE} style uses an internationally-distinct currency
 * code, such as {@code "TDC"}, to indicate the units of denomination, while the
 * {@code SYMBOL} style uses a possibly-ambiguous currency symbol such as
 * {@code "฿"}.</p>
 *
 * <p>The denomination used when formatting will be either tdcoin, millicoin
 ...
 * locale, a value of one tdcoin might be formatted as {@code ฿1.00} where a value
 ...
 * increasing precision to convert the representation of a given quantity of tdcoins into a
 * representation of the same value denominated in the formatter's units.  For example, a scale
 * value of {@code 3} specifies a denomination of millitdcoins, because to represent
 * {@code 1.0000 TDC}, or one tdcoin, in millitdcoins, one shifts the decimal point
 * three places, that is, to {@code 1000.0 mTDC}.</p>
 *
 * <h3>Construction</h3>
 *
 ...
 *   <li>Use a {@link TdcFormat.Builder} object.</ol>
 *
 ...
 * methods of this abstract base class {@link TdcFormat}.  There
 ...
 * automatic-denominating {@link TdcAutoFormat} instance for your
 ...
 * <blockquote><pre>
 * TdcFormat f = TdcFormat.getInstance();
 * String s = f.format(Coin.COIN); <strong>// "TDC 1.00"</strong>
 * </pre></blockquote>
 *
 ...
 * the {@link TdcFixedFormat} instance that is returned, and thus will
 * determine its denomination.  For example, if you want to format
 * values in units of microtdcoins:</p>
 *
 * <blockquote><pre>TdcFormat m = TdcFormat.getInstance(6);
 ...
 *
 * <blockquote><pre>TdcFormat milliFormat = TdcFormat.getInstance(MILLICOIN_SCALE);</pre></blockquote>
 *
 * <p>Alternatively, if the type of the first argument to
 * {@code getInstance()} is one of the enumerated values of the
 * {@link TdcAutoFormat.Style} type, either {@link TdcAutoFormat.Style#CODE} or
 * {@link TdcAutoFormat.Style#SYMBOL}, then you will get a {@link TdcAutoFormat}
 * instance that uses either a currency code or symbol, respectively,
 * to indicate the results of its auto-denomination.</p>
 *
 * <blockquote><pre>
 * TdcFormat s = TdcFormat.getInstance(SYMBOL);
 ...
 * differently the same one-tdcoin value:</p>
 *
 * <blockquote><pre>
 * <strong>// Next line returns "1,00 TDC"</strong>
 * TdcFormat.getInstance(Locale.GERMANY).format(Coin.COIN);
 * <strong>// Next line returns "1,00 ฿"</strong>
 * TdcFormat.getInstance(SYMBOL, Locale.GERMANY).format(Coin.COIN);
 * <strong>// Next line returns "1.000,00"</strong>
 * TdcFormat.getMilliInstance(Locale.GERMANY).format(Coin.COIN);
 * <strong>// Next line returns "10.000,00"</strong>
 * TdcFormat.getInstance(4, Locale.GERMANY).format(Coin.COIN);
 * </pre></blockquote>
 *
 ...
 *
 * <p>A new {@link TdcFormat.Builder} instance is returned by the {@link #builder()} method.  Such
 * an object has methods that set the configuration parameters of a {@link TdcFormat}
 * object.  Its {@link Builder#build()} method constructs and returns a {@link TdcFormat} instance
 ...
 *
 * <blockquote><pre>
 * TdcFormat f = TdcFormat.builder().style(CODE).code("XBT").build();
 * String out = f.format(COIN); <strong>// "XBT 1.00"</strong>
 * </pre></blockquote>
 *
 * <p>See the documentation of the {@link TdcFormat.Builder} class for details.</p>
 *
 * <h3>Formatting</h3>
 *
 * <p>You format a Tdcoin monetary value by passing it to the {@link TdcFormat#format(Object)}
 ...
 * number of tdcoins.  A value having a fractional amount of satoshis is rounded to the
 ...
 * {@link TdcFixedFormat} also accept a variable-length sequence of additional {@code int}
 ...
 *
 * <blockquote><pre>
 * TdcFormat f = TdcFormat.getCoinInstance();
 ...
 * {@link TdcFixedFormat#REPEATING_PLACES}, {@link
 * TdcFixedFormat#REPEATING_DOUBLETS} and {@link
 * TdcFixedFormat#REPEATING_TRIPLETS}.  These signify repeating groups
 ...
 *
 * <blockquote><pre>
 * TdcFormat de = TdcFormat.getInstance(Locale.GERMANY);
 * FieldPosition currField = new FieldPosition(NumberFormat.Field.CURRENCY);
 * <strong>// next line formats the value as "987.654.321,23 µTDC"</strong>
 * String output = de.format(valueOf(98765432123L), new StringBuffer(), currField);
 * <strong>// next line sets variable currencyCode to "µTDC"</strong>
 ...
 *
 * <blockquote><pre>
 * TdcFixedFormat kilo = (TdcFixedFormat)TdcFormat(-3); <strong>// scale -3 for kilocoins</strong>
 * Coin value = Coin.parseCoin("1230");
 * <strong>// variable coded will be set to "kTDC 1.23"</strong>
 ...
 * TdcFormat(4).code(); <strong>// unnamed denomination has no code; raises exception</strong>
 * </pre></blockquote>
 *
 ...
 *                SATOSHI};
 * TdcFormat f = TdcFormat.getCoinInstance(2, REPEATING_PLACES);
 ...
 * TdcFixedFormat} and {@link TdcAutoFormat} is analogous to the difference in formatting
 * behavior between instances of those classes.  Instances of {@link TdcAutoFormat} recognize
 ...
 * instances of {@link TdcFixedFormat} by default recognize no codes nor symbols, but rather
 ...
 * TdcFixedFormat} can be overridden by setting a parsing pattern that includes a currency sign
 * using the {@link TdcFormat.Builder#pattern()} method.</p>
 *
 * <p>The {@link TdcAutoFormat#parse(String)} method of {@link TdcAutoFormat} (and of
 * {@link TdcAutoFormat} configured with applicable non-default pattern) will recognize a
 ...
 * {@code µTDC} or other appropriate permutations of those characters.  Additionally, if
 * either or both of a custom currency code or symbol is configured using {@link
 * TdcFormat.Builder#code} or {@link TdcFormat.Builder#code}, then such code or symbol will
 ...
 * {@code TdcFormat} instance, on the other hand, whether or not the units indicator must
 * be separated by a space from the number is determined by the locale.  The {@link
 * TdcFormat#pattern()} method returns a representation of the pattern that
 * can be examined to determine whether a space must separate currency signs from numbers in
 * parsed {@link String}s.</p>
 *
 * <p>When parsing, if the currency-units indicator is absent, then a {@link TdcAutoFormat}
 * instance will infer a denomination of tdcoins while a {@link TdcFixedFormat} will infer the
...
public abstract class TdcFormat extends Format {
    ...
    /** The conventional international currency code for tdcoins: "TDC" */
    private static final String COIN_CODE = "TDC";
    /** The default currency symbols for tdcoins */
    ...
    /* Scale is the number of decimal-places difference from same value in tdcoins */
    /** A constant useful for specifying a denomination of tdcoins, the {@code int} value
     *  {@code 0}. */
    public static final int COIN_SCALE = 0;

    /** A constant useful for specifying a denomination of millitdcoins, the {@code int}
     *  value {@code 3}. */
    public static final int MILLICOIN_SCALE = 3;

    /** A constant useful for specifying a denomination of microtdcoins, the {@code int}
     *  value {@code 6}. */
    ...
    /**
     * <p>This class constructs new instances of {@link TdcFormat}, allowing for the
     * configuration of those instances before they are constructed.  After obtaining a
     * {@code Builder} object from the {@link TdcFormat#builder()} method, invoke the
     * necessary setter methods to obtain your desired configuration.  Finally, the {@link
     * #build()} method returns a new {@code TdcFormat} object that has the specified
     * configuration.
     *
     * <p>All the setter methods override defaults.  Invoking {@code build()} without invoking any
     * of the setting methods is equivalent to invoking {@link TdcFormat#getInstance()} with no arguments.
     *
    ...
    public static class Builder {
        ...
        private enum Variant {
            AUTO { @Override TdcFormat newInstance(Builder b) {
                       return getInstance(b.style, b.locale, b.minimumFractionDigits);
                   }},
            FIXED,
            UNSET;
            TdcFormat newInstance(Builder b) {
        ...
        private Style style = TdcAutoFormat.Style.CODE;
        private int scale = 0;
        private String symbol = "",code = "",pattern = "",localizedPattern = "";

        private Builder() {}

        /** Specify the new {@code TdcFormat} is to be automatically-denominating.
         * The argument determines which of either codes or symbols the new {@code TdcFormat}
        ...
        public Builder style(TdcAutoFormat.Style val) {
         ...
         * appropriately for the context of generating {@link TdcAutoFormat} instances.
         *
         ...
         *  denominated in tdcoins.  For example, a denomination of millitdcoins is specified
         *  with a value of {@code 3}.
         *
         * <p>The {@code TdcFormat} class provides appropriately named
         * {@code int}-type constants for the three common values, {@link TdcFormat#COIN_SCALE},
         * {@link TdcFormat#MILLICOIN_SCALE} {@link TdcFormat#MICROCOIN_SCALE}.
         *
         * <p>If neither this method nor {@link #style(TdcAutoFormat.Style)} is invoked on a
         * {@link Builder}, then the {@link TdcFormat} will default to a
         * fixed-denomination of tdcoins, equivalent to invoking this method with an argument
         * of {@code 0}. */
         ...
         *  class.
         *
         *  <p>The pattern is localized according to the locale of the {@code TdcFormat}
         *  instance, the symbols for which can be examined by inspecting the {@link
         *  java.text.DecimalFormatSymbols} object returned by {@link TdcFormat#symbols()}.
        ...
        /** Return a new {@link TdcFormat} instance.  The object returned will be configured according
         *  to the state of this {@code Builder} instance at the time this method is invoked. */
        public TdcFormat build() {
            TdcFormat f = variant.newInstance(this);
    ...
    protected TdcFormat(DecimalFormat numberFormat, int minDecimals, List<Integer> groups) {
     ...
     * currency code, for example "{@code TDC}", to indicate that denomination.  The
     ...
     */
    public static TdcFormat getInstance() { return getInstance(defaultLocale()); }
     ...
     */
    public static TdcFormat getSymbolInstance() { return getSymbolInstance(defaultLocale()); }

    /**
     * Return a new auto-denominating instance that will indicate units using a currency
     * code, for example, {@code "TDC"}.  Formatting and parsing will be done
     * according to the default locale.
     */
    public static TdcFormat getCodeInstance() { return getCodeInstance(defaultLocale()); }
     ...
     * locale.
     */
    public static TdcFormat getSymbolInstance(int fractionPlaces) {
     ...
     * {@code "TDC"}.  The returned object will format the fraction-part of numbers using
     * the given number of decimal places, or fewer as necessary to avoid giving a place to
     * fractional satoshis.  Formatting and parsing will be done according to the default
     * locale.
     */
    public static TdcFormat getCodeInstance(int minDecimals) {
     ...
     * units using a currency code, for example, {@code "mTDC"}.
     */
    public static TdcFormat getInstance(Locale locale) { return getCodeInstance(locale); }
     ...
     * units using a currency code, for example, {@code "mTDC"}.
     */
    public static TdcFormat getCodeInstance(Locale locale) { return getInstance(CODE, locale); }
     ...
     * {@code "mTDC"}.  The returned object will format the fraction-part of numbers using
     * the given number of decimal places, or fewer as necessary to avoid giving a place to
     * fractional satoshis.
     */
    public static TdcFormat getInstance(Locale locale, int minDecimals) {
     ...
     * fractional satoshis.
     */
    public static TdcFormat getCodeInstance(Locale locale, int minDecimals) {
     ...
     */
    public static TdcFormat getSymbolInstance(Locale locale) {
     ...
     * fractional satoshis.
     */
    public static TdcFormat getSymbolInstance(Locale locale, int fractionPlaces) {
     ...
     * {@code "฿"}, or code, such as {@code "mTDC"}, depending on the value of
     * the argument.  Formatting and parsing will be done according to the default locale.
     */
    public static TdcFormat getInstance(Style style) { return getInstance(style, defaultLocale()); }
     ...
     * {@code "mTDC"}, depending on the value of the first argument.  The returned object
     ...
     */
    public static TdcFormat getInstance(Style style, int fractionPlaces) {
     ...
     * {@code "TDC"}, or symbol, such as {@code "฿"}, depending on the value
     ...
     */
    public static TdcFormat getInstance(Style style, Locale locale) {
     ...
     * such as {@code "mTDC"}, or symbol, such as {@code "฿"},
     ...
     * fractional satoshis.
     */
    public static TdcFormat getInstance(Style style, Locale locale, int fractionPlaces) {
        return new TdcAutoFormat(locale, style, fractionPlaces);
    }
     ...
     */
    public static TdcFormat getCoinInstance() { return getCoinInstance(defaultLocale()); }
     ...
     */
    public static TdcFormat getCoinInstance(int minFractionPlaces, int... groups) {
     ...
     */
    public static TdcFormat getCoinInstance(Locale locale) {
        return getInstance(COIN_SCALE, locale, 2);
    }

    /**
     * Return a newly-constructed instance for the given locale that will format
     * values in terms of tdcoins, with the given minimum number of fractional
     ...
     */
    public static TdcFormat getCoinInstance(Locale locale, int scale, int... groups) {
     ...
     */
    public static TdcFormat getMilliInstance() { return getMilliInstance(defaultLocale()); }
     ...
     * necessary.
     */
    public static TdcFormat getMilliInstance(Locale locale) {
     ...
     */
    public static TdcFormat getMilliInstance(int scale, int... groups) {
     ...
     */
    public static TdcFormat getMilliInstance(Locale locale, int scale, int... groups) {
     ...
     * necessary.
     */
    public static TdcFormat getMicroInstance() { return getMicroInstance(defaultLocale()); }
     ...
     * necessary.
     */
    public static TdcFormat getMicroInstance(Locale locale) {
     ...
     * necessary to avoid rounding, to a limiting precision of satoshis.
     */
    public static TdcFormat getMicroInstance(int scale, int... groups) {
     ...
     * limiting precision of satoshis.
     */
    public static TdcFormat getMicroInstance(Locale locale, int scale, int... groups) {
     ...
     * limiting precision of satoshis.
     */
    public static TdcFormat getInstance(int scale, int minDecimals, int... groups) {
     ...
     */
    public static TdcFormat getInstance(int scale) {
     ...
     */
    public static TdcFormat getInstance(int scale, Locale locale) {
     ...
     */
    public static TdcFormat getInstance(int scale, Locale locale, int minDecimals, int... groups) {
     ...
     */
    public static TdcFormat getInstance(int scale, Locale locale, int minDecimals, List<Integer> groups) {
        return new TdcFixedFormat(locale, scale, minDecimals, groups);
    }

    // ****** FORMATTING *****

    /**
     * Formats a tdcoin monetary value and returns an {@link AttributedCharacterIterator}.
    ...
    /**
     * Formats a tdcoin value as a number and possibly a units indicator and appends the
     ...
     * number of tdcoins, rounded to the nearest satoshi as necessary.
     *
    ...
    /**
     * Formats a tdcoin value as a number and possibly a units indicator to a
     ...
     * {@link BigDecimal} is interpreted as that number of tdcoins, rounded to the
     ...
     * is the size of the decimal-place shift between the given Tdcoin-value denominated in
     * tdcoins and that same value as formatted.  A fixed-denomination formatter will ignore
     * the arguments.
     *
    ...
    /**
     * Takes a tdcoin monetary value that the client wants to format and returns the number of
    ...
    /**
     * Takes an object representing a tdcoin quantity of any type the
    ...
    private static BigInteger inSatoshis(Object qty) {
        BigInteger satoshis;
        // the value might be tdcoins or satoshis
    ...
    /**
      * Parse a {@link String} representation of a Tdcoin monetary value.  Returns a
    ...
    /** Parse a {@link String} representation of a Tdcoin monetary value.  If this
     * object's pattern includes a currency sign, either symbol or code, as by default is true
     * for instances of {@link TdcAutoFormat} and false for instances of {@link
     * TdcFixedFormat}, then denominated (i.e., prefixed) currency signs in the parsed String
     ...
     * String, then the number is interpreted as a quatity of tdcoins.
     * <p>If the pattern contains neither a currency symbol nor sign, then instances of {@link
     * TdcAutoFormat} will interpret the parsed number as a quantity of tdcoins, and instances
     * of {@link TdcAutoFormat} will interpret the number as a quantity of that instance's
     * configured denomination, which can be ascertained by invoking the {@link
     * TdcFixedFormat#symbol()} or {@link TdcFixedFormat#code()} method.
     *
    ...
    /** Return the unprefixed currency symbol for tdcoins configured for this object.  The
    ...
    /** Return the unprefixed international currency code for tdcoins configured for this
    ...
    @Override public boolean equals(Object o) {
        ...
        if (!(o instanceof TdcFormat)) return false;
        TdcFormat other = (TdcFormat)o;
