...
import org.tdcoinj.params.AbstractTdcoinNetParams;
...
/**
 * <p>Provides a standard implementation of a Tdcoin URI with support for the following:</p>
 *
 * <ul>
 ...
 *
 * <ul>
 * <li>{@code tdcoin:<address>}</li>
 * <li>{@code tdcoin:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
 ...
 * @see <a href="https://github.com/tdcoin/bips/blob/master/bip-0021.mediawiki">BIP 0021</a>
 */
public class TdcoinURI {
    ...
    /**
     * URI for Tdcoin network. Use {@link AbstractTdcoinNetParams#TDCOIN_SCHEME} if you specifically
     * need Tdcoin, or use {@link NetworkParameters#getUriScheme} to get the scheme
     * from network parameters.
     */
    @Deprecated
    public static final String TDCOIN_SCHEME = "tdcoin";
    ...
    /**
     * Constructs a new TdcoinURI from the given string. Can be for any network.
     *
     * @param uri The raw URI data to be parsed (see class comments for accepted formats)
     * @throws TdcoinURIParseException if the URI is not syntactically or semantically valid.
     */
    public TdcoinURI(String uri) throws TdcoinURIParseException {
        this(null, uri);
    }

    /**
     * Constructs a new object by trying to parse the input as a valid Tdcoin URI.
     *
     ...
     *
     * @throws TdcoinURIParseException If the input fails Tdcoin URI syntax and semantic checks.
     */
    public TdcoinURI(@Nullable NetworkParameters params, String input) throws TdcoinURIParseException {
        checkNotNull(input);

        String scheme = null == params
            ? AbstractTdcoinNetParams.TDCOIN_SCHEME
            : params.getUriScheme();
        ...
        try {
            ...
            throw new TdcoinURIParseException("Bad URI syntax", e);
        }

        // URI is formed as  tdcoin:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form tdcoin://address?....
        // We support both until Ben fixes his code.
        
        // Remove the tdcoin scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : tdcoin:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        ...
        if (inputLc.startsWith(blockchainInfoScheme)) {
            ...
            throw new TdcoinURIParseException("Unsupported URI scheme: " + uri.getScheme());
        }

        // Split off the address from the rest of the query parameters.
        String[] addressSplitTokens = schemeSpecificPart.split("\\?", 2);
        if (addressSplitTokens.length == 0)
            throw new TdcoinURIParseException("No data found after the tdcoin: prefix");
        ...
        if (!addressToken.isEmpty()) {
            // Attempt to parse the addressToken as a Tdcoin address for this network
            try {
                Address address = Address.fromString(params, addressToken);
                putWithValidation(FIELD_ADDRESS, address);
            } catch (final AddressFormatException e) {
                throw new TdcoinURIParseException("Bad address", e);
            }
        }

        if (addressToken.isEmpty() && getPaymentRequestUrl() == null) {
            throw new TdcoinURIParseException("No address and no r= parameter found");
        }
    }
     ...
     */
    private void parseParameters(@Nullable NetworkParameters params, String addressToken, String[] nameValuePairTokens) throws TdcoinURIParseException {
        // Attempt to decode the rest of the tokens into a parameter map.
        for (String nameValuePairToken : nameValuePairTokens) {
            final int sepIndex = nameValuePairToken.indexOf('=');
            if (sepIndex == -1)
                throw new TdcoinURIParseException("Malformed Tdcoin URI - no separator in '" +
                        nameValuePairToken + "'");
            if (sepIndex == 0)
                throw new TdcoinURIParseException("Malformed Tdcoin URI - empty name '" +
                        nameValuePairToken + "'");
            ...
            if (FIELD_AMOUNT.equals(nameToken)) {
                ...
                try {
                    ...
                    if (params != null && amount.isGreaterThan(params.getMaxMoney()))
                        throw new TdcoinURIParseException("Max number of coins exceeded");
                    if (amount.signum() < 0)
     ...
     */
    private void putWithValidation(String key, Object value) throws TdcoinURIParseException {
        if (parameterMap.containsKey(key)) {
            throw new TdcoinURIParseException(String.format(Locale.US, "'%s' is duplicated, URI is invalid", key));
        } else {
    ...
    /**
     * The Tdcoin address from the URI, if one was present. It's possible to have Tdcoin URI's with no address if a
     ...
     *         10,000,000 units is 1 TDC. May be null if no amount is specified
     */
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder("TdcoinURI[");
        boolean first = true;
    ...
    /**
     * Simple Tdcoin URI builder using known good fields.
     *
     * @param address The Tdcoin address
     * @param amount The amount
     * @param label A label
     * @param message A message
     * @return A String containing the Tdcoin URI
     */
    public static String convertToTdcoinURI(Address address, Coin amount,
                                             String label, String message) {
        return convertToTdcoinURI(address.getParameters(), address.toString(), amount, label, message);
    }
     ...
     * @return A String containing the Tdcoin URI
     */
    public static String convertToTdcoinURI(NetworkParameters params,
