...
import static org.tdcoinj.core.Coin.CENT;
import static org.tdcoinj.core.Coin.COIN;
import static org.tdcoinj.core.Coin.SATOSHI;
import static org.tdcoinj.core.Coin.ZERO;
...
public class MonetaryFormatTest {
    ...
    private static final MonetaryFormat NO_CODE = MonetaryFormat.TDC.noCode();
    ...
    @Test
    public void mTdcRounding() throws Exception {
    ...
    @Test
    public void uTdcRounding() throws Exception {
    ...
    public void standardCodes() throws Exception {
        assertEquals("TDC 0.00", MonetaryFormat.TDC.format(Coin.ZERO).toString());
        assertEquals("mTDC 0.00", MonetaryFormat.MTDC.format(Coin.ZERO).toString());
        assertEquals("µTDC 0", MonetaryFormat.UTDC.format(Coin.ZERO).toString());
    }

    @Test
    public void standardSymbol() throws Exception {
        assertEquals(MonetaryFormat.SYMBOL_TDC + " 0.00", new MonetaryFormat(true).format(Coin.ZERO).toString());
    }

    @Test
    public void customCode() throws Exception {
        assertEquals("dTDC 0", MonetaryFormat.UTDC.code(1, "dTDC").shift(1).format(Coin.ZERO).toString());
    }
    ...
    public void noCode() throws Exception {
        assertEquals("0", MonetaryFormat.UTDC.noCode().shift(0).format(Coin.ZERO).toString());
        // Ensure that inserting a code after codes are wiped, works
        assertEquals("dTDC 0", MonetaryFormat.UTDC.noCode().code(1, "dTDC").shift(1).format(Coin.ZERO).toString());
    }

    @Test
    public void codeOrientation() throws Exception {
        assertEquals("TDC 0.00", MonetaryFormat.TDC.prefixCode().format(Coin.ZERO).toString());
        assertEquals("0.00 TDC", MonetaryFormat.TDC.postfixCode().format(Coin.ZERO).toString());
    }

    @Test
    public void codeSeparator() throws Exception {
        assertEquals("TDC@0.00", MonetaryFormat.TDC.codeSeparator('@').format(Coin.ZERO).toString());
    }

    @Test(expected = NumberFormatException.class)
    public void missingCode() throws Exception {
        MonetaryFormat.UTDC.shift(1).format(Coin.ZERO);
    }
    ...
    public void parse() throws Exception {
        ...
        assertEquals(Coin.MILLICOIN, MonetaryFormat.MTDC.parse("1"));
        assertEquals(Coin.MILLICOIN, MonetaryFormat.MTDC.parse("1.0"));
        assertEquals(Coin.MILLICOIN, MonetaryFormat.MTDC.parse("01.0000000000"));
        assertEquals(Coin.MILLICOIN, MonetaryFormat.MTDC.positiveSign('+').parse("+1.0"));
        assertEquals(Coin.MILLICOIN.negate(), MonetaryFormat.MTDC.parse("-1"));
        assertEquals(Coin.MILLICOIN.negate(), MonetaryFormat.MTDC.parse("-1.0"));

        assertEquals(Coin.MICROCOIN, MonetaryFormat.UTDC.parse("1"));
        assertEquals(Coin.MICROCOIN, MonetaryFormat.UTDC.parse("1.0"));
        assertEquals(Coin.MICROCOIN, MonetaryFormat.UTDC.parse("01.0000000000"));
        assertEquals(Coin.MICROCOIN, MonetaryFormat.UTDC.positiveSign('+').parse("+1.0"));
        assertEquals(Coin.MICROCOIN.negate(), MonetaryFormat.UTDC.parse("-1"));
        assertEquals(Coin.MICROCOIN.negate(), MonetaryFormat.UTDC.parse("-1.0"));
