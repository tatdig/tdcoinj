...
import org.tdcoinj.script.ScriptChunk;
...
import org.tdcoinj.wallet.Protos.Wallet.EncryptionType;
...
import static org.tdcoinj.testing.FakeTxBuilder.*;
...
public class WalletTest extends TestWithWallet {
    ...
    private void basicSpendingCommon(Wallet wallet, Address toAddress, Address destination, Wallet encryptedWallet) throws Exception {
        ...
        // will attach a small fee. Because the Tdcoin protocol makes it difficult to determine the fee of an
    ...
    // identical result to Tdcoin Core. However the signatures are not deterministic - signing the same data
    ...
    public void blockChainCatchup() throws Exception {
        ...
        // What we'd really like to do is prove Tdcoin Core would accept it .... no such luck unfortunately.
    ...
    public void balances() throws Exception {
        ...
        Transaction send1 = wallet.createSend(OTHER_ADDRESS, valueOf(0, 10));
        // Reserialize.
        Transaction send2 = UNITTEST.getDefaultSerializer().makeTransaction(send1.tdcoinSerialize());
    ...
    public void isConsistent_duplicates() throws Exception {
        ...
        Transaction txClone = UNITTEST.getDefaultSerializer().makeTransaction(tx.tdcoinSerialize());
        try {
    ...
    public void doubleSpendUnspendsOtherInputs() throws Exception {
        ...
        send2 = UNITTEST.getDefaultSerializer().makeTransaction(send2.tdcoinSerialize());
    ...
    public void doubleSpends() throws Exception {
        ...
        byte[] buf = send1.tdcoinSerialize();
        buf[43] = 0;  // Break the signature: tdcoinj won't check in SPV mode and this is easier than other mutations.
    ...
    public void pending1() throws Exception {
        ...
        final Transaction t1Copy = UNITTEST.getDefaultSerializer().makeTransaction(t1.tdcoinSerialize());
    ...
    public void spendToSameWallet() throws Exception {
        ...
        assertEquals(ZERO, wallet.getBalance());
        sendMoneyToWallet(AbstractBlockChain.NewBlockType.BEST_CHAIN, outbound1);
        // We should have a balance of 1 TDC after the block is received.
        assertEquals(coin1, wallet.getBalance());
    }
    ...
    public void watchingScriptsBloomFilter() throws Exception {
        ...
        assertFalse(wallet.getBloomFilter(1e-12).contains(outPoint.unsafeTdcoinSerialize()));

        sendMoneyToWallet(BlockChain.NewBlockType.BEST_CHAIN, t1);
        assertTrue(wallet.getBloomFilter(1e-12).contains(outPoint.unsafeTdcoinSerialize()));
    }
    ...
    public void marriedKeychainBloomFilter() throws Exception {
        ...
        assertFalse(wallet.getBloomFilter(0.001).contains(outPoint.unsafeTdcoinSerialize()));

        sendMoneyToWallet(BlockChain.NewBlockType.BEST_CHAIN, t1);
        assertTrue(wallet.getBloomFilter(0.001).contains(outPoint.unsafeTdcoinSerialize()));
    }

    @Test
    public void autosaveImmediate() throws Exception {
        // Test that the wallet will save itself automatically when it changes.
        File f = File.createTempFile("tdcoinj-unit-test", null);
    ...
    public void opReturnTwoOutputsTest() throws Exception {
        // Tests sending transaction where one output transfers TDC, the other one writes OP_RETURN.
        receiveATransaction(wallet, myAddress);
        Transaction tx = new Transaction(UNITTEST);
        Coin messagePrice = Coin.ZERO;
        Script script = ScriptBuilder.createOpReturnScript("hello world!".getBytes());
    ...
    public void feeSolverAndCoinSelectionTests2() throws Exception {
        ...
        assertTrue(request15.tx.unsafeTdcoinSerialize().length > 1000);
        ...
        assertTrue(request16.tx.unsafeTdcoinSerialize().length > 1000);
        ...
        int theoreticalMaxLength17 = request17.tx.unsafeTdcoinSerialize().length + myKey.getPubKey().length + 75;
        ...
        Transaction spend17 = request17.tx;
        {
            ...
            final int length = spend17.unsafeTdcoinSerialize().length;
        ...
        int theoreticalMaxLength18 = spend18.unsafeTdcoinSerialize().length + myKey.getPubKey().length + 75;
        ...
        assertTrue(spend18.unsafeTdcoinSerialize().length >= 998);
        assertTrue(spend18.unsafeTdcoinSerialize().length <= 1001);
        ...
        request26.tx.addOutput(CENT.subtract(fee.add(dustThresholdMinusOne)),
                OTHER_ADDRESS);
        assertTrue(request26.tx.unsafeTdcoinSerialize().length > 1000);
    ...
    public void keyRotationHD2() throws Exception {
        ...
        assertEquals("n42QPxU8FUMfZhtCAjjXTb6DsuMSoFkQYM", LegacyAddress.fromKey(UNITTEST, usedKey).toString());
    ...
    public void completeTxPartiallySignedWithDummySigs() throws Exception {
        byte[] dummySig = TransactionSignature.dummy().encodeToTdcoin();
    ...
    public void totalReceivedSent() throws Exception {
        // Receive 4 TDC in 2 separate transactions
        ...
        // Send 3 TDC in a single transaction
