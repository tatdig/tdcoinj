...
import org.tdcoinj.core.Transaction.SigHash;
...
import static org.tdcoinj.script.ScriptOpCodes.*;
 ...
 *    rethink your life. Seriously, why are you reimplementing Tdcoin consensus rules? Instead, go work on making
 *    Tdcoin Core consensus rules a shared library and use that. Seriously, you wont get it right, and starting with
 *    this tester as a way to try to do so will simply end in pain and lost coins. SERIOUSLY, JUST STOP!
 *
 * b) Tdcoin Core is failing some test in here and you're wondering what test is causing failure. Just stop. There is no
...
public class FullBlockTestGenerator {
    // Used by TdcoindComparisonTool and AbstractFullPrunedBlockChainTest to create test cases
    ...
    public RuleList getBlocksToTest(boolean runBarelyExpensiveTests, boolean runExpensiveTests, File blockStorageFile) throws ScriptException, ProtocolException, IOException {
            ...
            public boolean add(Rule element) {
                if (outStream != null && element instanceof BlockAndValidity) {
                    try {
                        ...
                        byte[] block = ((BlockAndValidity)element).block.tdcoinSerialize();
        ...
        // Extend the b24 chain to make sure tdcoind isn't accepting b24
        ...
        Block b56;
        try {
            b56 = params.getDefaultSerializer().makeBlock(b57.block.tdcoinSerialize());
        ...
        Block b56p2;
        try {
            b56p2 = params.getDefaultSerializer().makeBlock(b57p2.block.tdcoinSerialize());
        } catch (ProtocolException e) {
            throw new RuntimeException(e); // Cannot happen.
        }
        ...
        Block b64; NewBlock b64Original;
        {
            ...
            for (Transaction transaction : b64Original.block.getTransactions())
                transaction.tdcoinSerialize(stream);
            ...
            checkState(Arrays.equals(stream.toByteArray(), b64.tdcoinSerialize()));
        ...
        Block b71 = params.getDefaultSerializer().makeBlock(b72.block.tdcoinSerialize());
    ...
    class BlockAndValidity extends Rule {
        ...
        boolean sendOnce; // We can throw away the memory for this block once we send it the first time (if tdcoind asks again, its broken)
