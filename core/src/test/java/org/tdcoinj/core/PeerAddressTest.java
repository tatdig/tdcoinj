...
public class PeerAddressTest {
    ...
    public void parse_ancientProtocolVersion() throws Exception {
        // copied from https://en.tdcoin.it/wiki/Protocol_documentation#Network_address
    ...
    @Test
    public void tdcoinSerialize_ancientProtocolVersion() throws Exception {
        PeerAddress pa = new PeerAddress(MAINNET, InetAddress.getByName(null), 9901, 0, BigInteger.ZERO);
        assertEquals(26, pa.length);        
        assertEquals("000000000000000000000000000000000000ffff7f00000126ad", Utils.HEX.encode(pa.tdcoinSerialize()));
    }
    ...
    public void roundtrip_ipv4_currentProtocolVersion() throws Exception {
        ...
        PeerAddress pa = new PeerAddress(MAINNET, InetAddress.getByName("1.2.3.4"), 1234,
                NetworkParameters.ProtocolVersion.CURRENT.getTdcoinProtocolVersion(), BigInteger.ZERO);
        byte[] serialized = pa.tdcoinSerialize();
        PeerAddress pa2 = new PeerAddress(MAINNET, serialized, 0,
                NetworkParameters.ProtocolVersion.CURRENT.getTdcoinProtocolVersion());
