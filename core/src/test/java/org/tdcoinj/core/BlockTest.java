...
import org.tdcoinj.core.AbstractBlockChain.NewBlockType;
...
public class BlockTest {
    ...
    public void testWork() throws Exception {
        ...
        // This number is printed by Tdcoin Core at startup as the calculated value of chainWork on testnet:
    ...
    public void testHeaderParse() throws Exception {
        ...
        Block reparsed = TESTNET.getDefaultSerializer().makeBlock(header.tdcoinSerialize());
        assertEquals(reparsed, header);
    }

    @Test
    public void testTdcoinSerialization() throws Exception {
        ...
        //
        // NB: This tests the tdcoin serialization protocol.
        assertArrayEquals(block700000Bytes, block700000.tdcoinSerialize());
    }
    ...
    public void testUpdateLength() {
        ...
        assertEquals(block.tdcoinSerialize().length, block.length);
        final int origBlockLen = block.length;
        Transaction tx = new Transaction(UNITTEST);
        // this is broken until the transaction has > 1 input + output (which is required anyway...)
        //assertTrue(tx.length == tx.tdcoinSerialize().length && tx.length == 8);
        ...
        assertEquals(tx.unsafeTdcoinSerialize().length, tx.length);
        assertEquals(origTxLength, tx.length);
        block.addTransaction(tx);
        assertEquals(block.unsafeTdcoinSerialize().length, block.length);
        ...
        assertEquals(block.length, block.unsafeTdcoinSerialize().length);
        assertEquals(block.length, origBlockLen + tx.length);
        assertEquals(tx.length, origTxLength - 1);
    ...
    public void parseBlockWithHugeDeclaredTransactionsSize() throws Exception{
        Block block = new Block(UNITTEST, 1, Sha256Hash.ZERO_HASH, Sha256Hash.ZERO_HASH, 1, 1, 1, new ArrayList<Transaction>()) {
            ...
            public byte[] tdcoinSerialize() {
                ...
                try {
                    tdcoinSerializeToStream(baos);
                } catch (IOException e) {
                }
                return baos.toByteArray();
            }
        };
        byte[] serializedBlock = block.tdcoinSerialize();
        try {
