...
import org.tdcoinj.core.listeners.*;
import org.tdcoinj.net.*;
import org.tdcoinj.params.*;
import org.tdcoinj.store.*;
import org.tdcoinj.utils.*;
import org.slf4j.*;
...
/**
 * A tool for comparing the blocks which are accepted/rejected by tdcoind/tdcoinj
 * It is designed to run as a testnet-in-a-box network between a single tdcoind node and tdcoinj
 * It is not an automated unit-test because it requires a bit more set-up...read comments below
 */
public class TdcoindComparisonTool {
    private static final Logger log = LoggerFactory.getLogger(TdcoindComparisonTool.class);

    private static final NetworkParameters PARAMS = RegTestParams.get();
    private static FullPrunedBlockChain chain;
    private static Sha256Hash tdcoindChainHead;
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("USAGE: tdcoinjBlockStoreLocation runExpensiveTests(1/0) [port=18444]");
        ...
        try {
            H2FullPrunedBlockStore store = new H2FullPrunedBlockStore(PARAMS, args.length > 0 ? args[0] : "TdcoindComparisonTool", blockList.maximumReorgBlockCount);
            store.resetStore();
        ...
        final Peer tdcoind = new Peer(PARAMS, ver, new BlockChain(PARAMS, new MemoryBlockStore(PARAMS)), new PeerAddress(PARAMS, InetAddress.getLocalHost()));
        Preconditions.checkState(tdcoind.getVersionMessage().hasBlockChain());
        ...
        tdcoind.addConnectedEventListener(Threading.SAME_THREAD, new PeerConnectedEventListener() {
            @Override
            public void onPeerConnected(Peer peer, int peerCount) {
                if (!peer.getPeerVersionMessage().subVer.contains("Satoshi")) {
                    ...
                    System.out.println("************************************************************************************************************************\n" +
                                       ...
                                       "think hard about what you're doing. Seriously, no one has gotten even close to correctly reimplementing Tdcoin\n" +
                                       "consensus rules, despite serious investment in trying. It is a huge task and the slightest difference is a huge bug.\n" +
                                       "Instead, go work on making Tdcoin Core consensus rules a shared library and use that. Seriously, you wont get it right,\n" +
                                       "and starting with this tester as a way to try to do so will simply end in pain and lost coins.\n" +
                                       "************************************************************************************************************************");
                    System.out.println();
                }
                log.info("tdcoind connected");
                // Make sure tdcoind has no blocks
                tdcoind.setDownloadParameters(0, false);
                tdcoind.startBlockChainDownload();
                connectedFuture.set(null);
            }
        });

        tdcoind.addDisconnectedEventListener(Threading.SAME_THREAD, new PeerDisconnectedEventListener() {
            @Override
            public void onPeerDisconnected(Peer peer, int peerCount) {
                log.error("tdcoind node disconnected!");
                System.exit(1);
            }
        });

        tdcoind.addPreMessageReceivedEventListener(Threading.SAME_THREAD, new PreMessageReceivedEventListener() {
            @Override
            public Message onPreMessageReceived(Peer peer, Message m) {
                if (m instanceof HeadersMessage) {
                    if (!((HeadersMessage) m).getBlockHeaders().isEmpty()) {
                        ...
                        log.info("Got header from tdcoind " + b.getHashAsString());
                        tdcoindChainHead = b.getHash();
                    } else
                        log.info("Got empty header message from tdcoind");
                    return null;
                } else if (m instanceof Block) {
                    log.error("tdcoind sent us a block it already had, make sure tdcoind has no blocks!");
                    System.exit(1);
                    ...
                    for (InventoryItem item : ((GetDataMessage) m).items)
                        if (item.type == InventoryItem.Type.BLOCK) {
                            ...
                            if (currentBlock.block.getHash().equals(item.hash))
                                tdcoind.sendMessage(currentBlock.block);
                            else {
                                Block nextBlock = preloadedBlocks.get(item.hash);
                                if (nextBlock != null)
                                    tdcoind.sendMessage(nextBlock);
                                else {
                    ...
                    try {
                        ...
                        if (!found)
                            sendHeaders = headers;
                        tdcoind.sendMessage(new HeadersMessage(PARAMS, sendHeaders));
                        InventoryMessage i = new InventoryMessage(PARAMS);
                        for (Block b : sendHeaders)
                            i.addBlock(b);
                        tdcoind.sendMessage(i);
                    } catch (Exception e) {
        ...
        tdcoindChainHead = PARAMS.getGenesisBlock().getHash();
        
        // tdcoind MUST be on localhost or we will get banned as a DoSer
        new NioClient(new InetSocketAddress(InetAddress.getLoopbackAddress(), args.length > 2 ? Integer.parseInt(args[2]) : PARAMS.getPort()), tdcoind, 1000);
        ...
        for (Rule rule : blockList.list) {
            if (rule instanceof FullBlockTestGenerator.BlockAndValidity) {
                ...
                // tdcoind doesn't request blocks inline so we can't rely on a ping for synchronization
                for (int i = 0; !shouldntRequest && !blocksRequested.contains(nextBlock.getHash()); i++) {
                    int SLEEP_TIME = 1;
                    if (i % 1000/SLEEP_TIME == 1000/SLEEP_TIME - 1)
                        log.error("tdcoind still hasn't requested block " + block.ruleName + " with hash " + nextBlock.getHash());
                    Thread.sleep(SLEEP_TIME);
                    if (i > 60000/SLEEP_TIME) {
                        log.error("tdcoind failed to request block " + block.ruleName);
                        System.exit(1);
                    }
                }
                if (shouldntRequest) {
                    ...
                    if (blocksRequested.contains(nextBlock.getHash())) {
                        log.error("ERROR: tdcoind re-requested block " + block.ruleName + " with hash " + nextBlock.getHash());
                        rulesSinceFirstFail++;
                    }
                }
                // If the block throws, we may want to get tdcoind to request the same block again
                if (block.throwsException)
                    blocksRequested.remove(nextBlock.getHash());
                //tdcoind.sendMessage(nextBlock);
                locator = new BlockLocator();
                locator = locator.add(tdcoindChainHead);
                tdcoind.sendMessage(new GetHeadersMessage(PARAMS, locator, hashTo));
                tdcoind.ping().get();
                if (!chain.getChainHead().getHeader().getHash().equals(tdcoindChainHead)) {
                    rulesSinceFirstFail++;
                    log.error("ERROR: tdcoind and tdcoinj acceptance differs on block \"" + block.ruleName + "\"");
                }
                if (block.sendOnce)
                ...
                if (mostRecentInv == null && !((MemoryPoolState) rule).mempool.isEmpty()) {
                    log.error("ERROR: tdcoind had an empty mempool, but we expected some transactions on rule " + rule.ruleName);
                    rulesSinceFirstFail++;
                } else if (mostRecentInv != null && ((MemoryPoolState) rule).mempool.isEmpty()) {
                    log.error("ERROR: tdcoind had a non-empty mempool, but we expected an empty one on rule " + rule.ruleName);
                    rulesSinceFirstFail++;
                    ...
                    if (matches)
                        continue;
                    log.error("tdcoind's mempool didn't match what we were expecting on rule " + rule.ruleName);
                    log.info("  tdcoind's mempool was: ");
                    for (InventoryItem item : mostRecentInv.items)
                        log.info("    " + item.hash);
                ...
                if (tdcoind.getPeerVersionMessage().isGetUTXOsSupported()) {
                    UTXORule r = (UTXORule) rule;
                    UTXOsMessage result = tdcoind.getUTXOs(r.query).get();
        ...
        if (unexpectedInvs.get() > 0)
            log.error("ERROR: Got " + unexpectedInvs.get() + " unexpected invs from tdcoind");
