...
public class VersionMessageTest {
    ...
    public void roundTrip_ipv4_currentProtocolVersion() throws Exception {
        ...
        ver.time = 23456;
        ver.subVer = "/tdcoinj/";
        ver.clientVersion = NetworkParameters.ProtocolVersion.CURRENT.getTdcoinProtocolVersion();
        ver.localServices = 1;
        ver.fromAddr = new PeerAddress(UNITTEST, InetAddress.getByName("1.2.3.4"), 3888);
        ver.fromAddr.setParent(ver);
        ver.receivingAddr = new PeerAddress(UNITTEST, InetAddress.getByName("4.3.2.1"), 9901);
        ver.receivingAddr.setParent(ver);
        byte[] serialized = ver.tdcoinSerialize();
        VersionMessage ver2 = new VersionMessage(UNITTEST, serialized);
        assertEquals(1234, ver2.bestHeight);
        assertEquals(23456, ver2.time);
        assertEquals("/tdcoinj/", ver2.subVer);
        assertEquals(NetworkParameters.ProtocolVersion.CURRENT.getTdcoinProtocolVersion(), ver2.clientVersion);
        ...
        assertEquals(9901, ver2.receivingAddr.getPort());
    }
    ...
    public void roundTrip_ipv6_currentProtocolVersion() throws Exception {
        ...
        ver.fromAddr.setParent(ver);
        ver.receivingAddr = new PeerAddress(UNITTEST, InetAddress.getByName("2002:db8:85a3:0:0:8a2e:370:7335"), 9901);
        ver.receivingAddr.setParent(ver);
        byte[] serialized = ver.tdcoinSerialize();
        VersionMessage ver2 = new VersionMessage(UNITTEST, serialized);
        assertEquals(1234, ver2.bestHeight);
