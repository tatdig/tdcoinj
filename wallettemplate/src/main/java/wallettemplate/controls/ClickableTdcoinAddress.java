...
/**
 * A custom control that implements a clickable, copyable Tdcoin address. Clicking it opens a local wallet app. The
 * address looks like a blue hyperlink. Next to it there are two icons, one that copies to the clipboard and another
 * that shows a QRcode.
 */
public class ClickableTdcoinAddress extends AnchorPane {
    ...
    public ClickableTdcoinAddress() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("tdcoin_address.fxml"));
            loader.setRoot(this);
    ...
    public String uri() {
        return TdcoinURI.convertToTdcoinURI(address.get(), null, Main.APP_NAME, null);
    }
    ...
    protected void showQRCode(MouseEvent event) {
        ...
        final Main.OverlayUI<ClickableTdcoinAddress> overlay = Main.instance.overlayUI(pane, this);
