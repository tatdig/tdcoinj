...
public class Main extends Application {
    ...
    public static WalletAppKit tdcoin;
    ...
    private void realStart(Stage mainWindow) throws IOException {
        ...
        // Tell tdcoinj to run event handlers on the JavaFX UI thread. This keeps things simple and means
        ...
        tdcoin.addListener(new Service.Listener() {
            @Override
        ...
        scene.getAccelerators().put(KeyCombination.valueOf("Shortcut+F"), () -> tdcoin.peerGroup().getDownloadPeer().close());
    }

    public void setupWalletKit(@Nullable DeterministicSeed seed) {
        // If seed is non-null it means we are restoring from backup.
        File appDataDirectory = AppDataDirectory.get(APP_NAME).toFile();
        tdcoin = new WalletAppKit(params, PREFERRED_OUTPUT_SCRIPT_TYPE, null, appDataDirectory, WALLET_FILE_NAME) {
            @Override
            protected void onSetupCompleted() {
                // Don't make the user wait for confirmations for now, as the intention is they're sending it
                // their own money!
                tdcoin.wallet().allowSpendingUnconfirmedTransactions();
                Platform.runLater(controller::onTdcoinSetup);
            }
        };
        // Now configure and start the appkit. This will take a second or two - we could show a temporary splash screen
        // or progress widget to keep the user engaged whilst we initialise, but we don't.
        if (params == RegTestParams.get()) {
            tdcoin.connectToLocalHost();   // You should run a regtest mode tdcoind locally.
        }
        tdcoin.setDownloadListener(controller.progressBarUpdater())
               .setBlockingStartup(false)
               .setUserAgent(APP_NAME, "1.0");
        if (seed != null)
            tdcoin.restoreWalletFromSeed(seed);
    }
