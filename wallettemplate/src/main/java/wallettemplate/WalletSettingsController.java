...
public class WalletSettingsController {
    ...
    public void initialize(@Nullable KeyParameter aesKey) {
        DeterministicSeed seed = Main.tdcoin.wallet().getKeyChainSeed();
        if (aesKey == null) {
            if (seed.isEncrypted()) {
            ...
            this.aesKey = aesKey;
            seed = seed.decrypt(checkNotNull(Main.tdcoin.wallet().getKeyCrypter()), "", aesKey);
    ...
    public void restoreClicked(ActionEvent event) {
        ...
        if (Main.tdcoin.wallet().getBalance().value > 0) {
        ...
        informationalAlert("Wallet restore in progress",
                "Your wallet will now be resynced from the Tdcoin network. This can take a long time for old wallets.");
        overlayUI.done();
        ...
        // Shut down tdcoinj and restart it with the new seed.
        Main.tdcoin.addListener(new Service.Listener() {
            @Override
            public void terminated(Service.State from) {
                Main.instance.setupWalletKit(seed);
                Main.tdcoin.startAsync();
            }
        }, Platform::runLater);
        Main.tdcoin.stopAsync();
    }
    ...
    public void passwordButtonClicked(ActionEvent event) {
        if (aesKey == null) {
            ...
            Main.tdcoin.wallet().decrypt(aesKey);
