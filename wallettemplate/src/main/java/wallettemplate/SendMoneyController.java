...
import wallettemplate.controls.TdcoinAddressValidator;
...
public class SendMoneyController {
    ...
    public void initialize() {
        Coin balance = Main.tdcoin.wallet().getBalance();
        checkState(!balance.isZero());
        new TdcoinAddressValidator(Main.params, address, sendBtn);
    ...
    public void send(ActionEvent event) {
        ...
        try {
            ...
            SendRequest req;
            if (amount.equals(Main.tdcoin.wallet().getBalance()))
                req = SendRequest.emptyWallet(destination);
            else
                req = SendRequest.to(destination, amount);
            req.aesKey = aesKey;
            sendResult = Main.tdcoin.wallet().sendCoins(req);
            ...
            ((HBox)tdcLabel.getParent()).getChildren().remove(tdcLabel);
            updateTitleForBroadcast();
