...
- To simplify tdcoinj programming in future.
...
Since the very first version tdcoinj has had the concept of "network parameters": a class that wrapped various
constants and magic numbers that distinguish the Tdcoin main network from the test network, and later, from settings
...
This proliferation of global variables makes it harder for developers to use multiple instances of tdcoinj
simultaneously, for example, to do cross-chain trading of different cryptocurrencies against each other.

Finally, several tdcoinj objects currently need to be plugged together in ways that aren't always obvious for full
...
NetworkParameters appears everywhere in the tdcoinj API, and so introducing Context will have a major impact on it
